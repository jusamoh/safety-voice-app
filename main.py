...
import websockets
# ✅ 수정됨: UploadFile, File 모듈 임포트 추가 (서버 크래시 및 마이크 튕김 해결)
...
class ConnectionManager:
    def __init__(self):
        ...
        self.global_document_context = "" # ✅ 수정됨: 문서 컨텍스트 전역 변수 추가
...
async def translate_and_send(text: str, source_lang: str, targets: str, recent_history: list, summary_state: dict, glossary_text: str, msg_id: str, role: str, name: str):
    try:
        ...
        # ✅ 수정됨: 사전 학습 문서를 시스템 프롬프트에 주입할 수 있도록 변수 복구
        ...
        # 🌟 극강의 정확도를 위한 관로 매핑 엔지니어링 특화 프롬프트 및 20개 필수 제어 규칙
        ...
        while True:
            ...
            if not is_speaker:
                ...
                if engine_mode == "azure":
                    ...
                    # ✅ 수정됨: STT 엔진 오류를 원천 차단하기 위한 강제 치환 규칙 복구
...
if __name__ == "__main__":
    ...
    print("🚀 실시간 글로벌 통역 서버 (Hybrid 엔진)를 시작합니다... ([http://0.0.0.0:10000](http://0.0.0.0:10000))")
